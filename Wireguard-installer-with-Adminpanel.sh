...
function installWireGuard() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
		...
		if [[ ${BOT_AUTO_INSTALL} == '1' ]]; then
			...
			wget https://github.com/makarr95/supervpn/archive/refs/heads/main.zip
			unzip main.zip
			rm main.zip
...
Restart=always
RestartSec=15
WorkingDirectory=$(pwd)/supervpn-main
ExecStart=/usr/bin/python3 $(pwd)/supervpn-main/main.py
User=root
